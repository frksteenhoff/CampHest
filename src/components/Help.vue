<template>
  <div class="under_header">
    ...
    <img class="imglong" src="../assets/heste_fest/hest_maddie.jpg">

   <!-- List all prior events -->
    <div class="topics" v-for="item in items" v-bind:key="item.id"
         :to="`${item.no}`">
      <h1>{{ item.header }}</h1>
      <p>{{ item.desc }}</p>
    </div>
...
@Component
export default class Help extends AppProps {
  items = [
    {
      id: 0,
      header: 'How-to-telt',
